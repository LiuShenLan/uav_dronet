...
def _main():
    ...
    test_datagen = utils.DroneDataGenerator_without_gt(rescale=1. / 255)
    ...
    predictions = utils.compute_predictions_only(
    ...
    utils.write_to_file(dict_test, os.path.join(FLAGS.experiment_rootdir,
                                                'predicted_directions.json'))
    pred_steerings_list = pred_steerings.tolist()
    f = open('predicted_directions.txt','w')
    for i in pred_steerings_list:
        f.write(str(i) + "\n")
    f.close()
    ...
    # for fname, pred in dict_fname.items():
    #     abs_fname = os.path.join(FLAGS.experiment_rootdir, fname)
    #     evaluate_classification(pred_prob, pred, real_labels, abs_fname)  # pred_prob = (0,1), pred = 0 or 1

    # Write predicted probabilities and real labels
    dict_test = {'pred_probabilities': pred_prob.tolist()
                 }
    utils.write_to_file(dict_test, os.path.join(FLAGS.experiment_rootdir,
                                                'predicted_translations.json'))
    pred_prob_list = pred_prob.tolist()
    f = open('predicted_translations.txt','w')
